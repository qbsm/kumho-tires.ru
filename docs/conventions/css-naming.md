...
- **База: `1rem = 10px`.** Корневой размер задан в `assets/css/base/typography.css`: `html { font-size: 10px; }`. Это делает арифметику в `rem` тривиальной: `1.6rem = 16px`, `2.4rem = 24px`, `0.8rem = 8px`. Считать дизайн-макет в `px` и делить на 10.
